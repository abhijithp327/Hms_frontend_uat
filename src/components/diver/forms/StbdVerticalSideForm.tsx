...
import { stbdVerticalSideSchema } from "@/validationSchema/diver/stbdVerticalSideSchema";

interface Props {
    data: FormData["stbdVerticalSide"];
    onChange: (data: FormData["stbdVerticalSide"]) => void;
    onValidationChange?: (isValid: boolean, errors: Record<string, string>) => void;
}

const StbdVerticalSideForm: React.FC<Props> = ({ data, onChange, onValidationChange }) => {
    ...
    const validateForm = (formData: FormData['stbdVerticalSide']) => {
        const result = stbdVerticalSideSchema.safeParse(formData);
    ...
    const handleChange = (field: keyof FormData['stbdVerticalSide'], value: string | number | string[]) => {
    ...
    return (
        <div className="space-y-6 pb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <MultipleImageUpload
                    id="stbdVerticalSideNearToTheFwdAreaBeforeCleaning"
                    label="Stbd vertical side area of the bow looking fwd before cleaning"
                    images={data.stbdVerticalSideNearToTheFwdAreaBeforeCleaning || []}
                    onImagesChange={(images) => handleChange('stbdVerticalSideNearToTheFwdAreaBeforeCleaning', images)}
                    maxImages={10}
                />

                <MultipleImageUpload
                    id="stbdVerticalSideNearToTheFwdAreaAfterCleaning"
                    label="Stbd vertical side area of the bow looking fwd after cleaning"
                    images={data.stbdVerticalSideNearToTheFwdAreaAfterCleaning || []}
                    onImagesChange={(images) => handleChange('stbdVerticalSideNearToTheFwdAreaAfterCleaning', images)}
                    maxImages={10}
                />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <MultipleImageUpload
                    id="stbdVerticalSideNearToTheMidAreaBeforeCleaning"
                    label="Port vertical side near to the mid area before cleaning"
                    images={data.stbdVerticalSideNearToTheMidAreaBeforeCleaning || []}
                    onImagesChange={(images) => handleChange('stbdVerticalSideNearToTheMidAreaBeforeCleaning', images)}
                    maxImages={10}
                />

                <MultipleImageUpload
                    id="stbdVerticalSideNearToTheMidAreaAfterCleaning"
                    label="Port vertical side near to the mid area after cleaning"
                    images={data.stbdVerticalSideNearToTheMidAreaAfterCleaning || []}
                    onImagesChange={(images) => handleChange('stbdVerticalSideNearToTheMidAreaAfterCleaning', images)}
                    maxImages={10}
                />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <MultipleImageUpload
                    id="stbdVerticalSideNearToTheAftAreaBeforeCleaning"
                    label="Port vertical side near to the aft area before cleaning"
                    images={data.stbdVerticalSideNearToTheAftAreaBeforeCleaning || []}
                    onImagesChange={(images) => handleChange('stbdVerticalSideNearToTheAftAreaBeforeCleaning', images)}
                    maxImages={10}
                />

                <MultipleImageUpload
                    id="stbdVerticalSideNearToTheAftAreaAfterCleaning"
                    label="Port vertical side near to the aft area after cleaning"
                    images={data.stbdVerticalSideNearToTheAftAreaAfterCleaning || []}
                    onImagesChange={(images) => handleChange('stbdVerticalSideNearToTheAftAreaAfterCleaning', images)}
                    maxImages={10}
                />
            </div>
...
export default StbdVerticalSideForm;