...
import { ropeGuardSchema } from "@/validationSchema/diver/ropeGuardSchema";

interface Props {
    data: FormData["ropeGuard"];
    onChange: (data: FormData["ropeGuard"]) => void;
    onValidationChange?: (isValid: boolean, errors: Record<string, string>) => void;
}

const RopeGuardForm: React.FC<Props> = ({ data, onChange, onValidationChange }) => {
    ...
    const validateForm = (formData: FormData['ropeGuard']) => {
        const result = ropeGuardSchema.safeParse(formData);
    ...
    const handleChange = (field: keyof FormData['ropeGuard'], value: string | number | string[]) => {
    ...
    return (
        <div className="space-y-6 pb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <Label htmlFor="plateAssembly">Plate Assembly</Label>
                    <Select value={data.plateAssembly} onValueChange={(value) => handleChange('plateAssembly', value)}>
                        <SelectTrigger className="mt-1">
                            <SelectValue placeholder="Select Plate Assembly" />
                        </SelectTrigger>
                        <SelectContent className="z-50 bg-white shadow-md" position="popper">
                ...
                <div>
                    <Label htmlFor="plateCondition">Weld Condition</Label>
                    <Select value={data.weldCondition} onValueChange={(value) => handleChange('weldCondition', value)}>
                        <SelectTrigger className="mt-1">
                            <SelectValue placeholder="Select Paint Condition" />
                        </SelectTrigger>
                        <SelectContent className="z-50 bg-white shadow-md" position="popper">
                ...
                <div>
                    <Label htmlFor="ropeNetCable">Rope /Net /Cable</Label>
                    <Select value={data.ropeNetCable} onValueChange={(value) => handleChange('ropeNetCable', value)}>
                        <SelectTrigger className="mt-1">
                            <SelectValue placeholder="Select Paint Condition" />
                        </SelectTrigger>
                        <SelectContent className="z-50 bg-white shadow-md" position="popper">
                            <SelectItem className="hover:bg-gray-100 cursor-pointer" value="clear">Clear</SelectItem>
                            <SelectItem className="hover:bg-gray-100 cursor-pointer" value="present">Present</SelectItem>
                        </SelectContent>
                    </Select>
                    {/* {errors.shellPlateCondition && <p className="text-sm text-red-600 mt-1">{errors.shellPlateCondition}</p>} */}
                </div>
