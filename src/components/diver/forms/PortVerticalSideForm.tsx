...
import { portVerticalSideSchema } from "@/validationSchema/diver/portVerticalSideSchema";
import MultipleImageUpload from "@/components/MultipleImageUploadSection";

interface Props {
    data: FormData["portVerticalSide"];
    onChange: (data: FormData["portVerticalSide"]) => void;
    onValidationChange?: (isValid: boolean, errors: Record<string, string>) => void;
}

const PortVerticalSideForm: React.FC<Props> = ({ data, onChange, onValidationChange }) => {
    ...
    const validateForm = (formData: FormData['portVerticalSide']) => {
        const result = portVerticalSideSchema.safeParse(formData);
    ...
    const handleChange = (field: keyof FormData['portVerticalSide'], value: string | number | string[]) => {
    ...
    return (
        <div className="space-y-6 pb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <MultipleImageUpload
                    id="portVerticalSideNearToTheFwdAreaBeforeCleaning"
                    label="Port vertical side area of the bow looking fwd before cleaning"
                    images={data.portVerticalSideNearToTheFwdAreaBeforeCleaning || []}
                    onImagesChange={(images) => handleChange('portVerticalSideNearToTheFwdAreaBeforeCleaning', images)}
                    maxImages={10}
                />

                <MultipleImageUpload
                    id="portVerticalSideNearToTheFwdAreaAfterCleaning"
                    label="Port vertical side area of the bow looking fwd after cleaning"
                    images={data.portVerticalSideNearToTheFwdAreaAfterCleaning || []}
                    onImagesChange={(images) => handleChange('portVerticalSideNearToTheFwdAreaAfterCleaning', images)}
                    maxImages={10}
                />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <MultipleImageUpload
                    id="portVerticalSideNearToTheMidAreaBeforeCleaning"
                    label="Port vertical side near to the mid area before cleaning"
                    images={data.portVerticalSideNearToTheMidAreaBeforeCleaning || []}
                    onImagesChange={(images) => handleChange('portVerticalSideNearToTheMidAreaBeforeCleaning', images)}
                    maxImages={10}
                />

                <MultipleImageUpload
                    id="portVerticalSideNearToTheMidAreaAfterCleaning"
                    label="Port vertical side near to the mid area after cleaning"
                    images={data.portVerticalSideNearToTheMidAreaAfterCleaning || []}
                    onImagesChange={(images) => handleChange('portVerticalSideNearToTheMidAreaAfterCleaning', images)}
                    maxImages={10}
                />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <MultipleImageUpload
                    id="portVerticalSideNearToTheAftAreaBeforeCleaning"
                    label="Port vertical side near to the aft area before cleaning"
                    images={data.portVerticalSideNearToTheAftAreaBeforeCleaning || []}
                    onImagesChange={(images) => handleChange('portVerticalSideNearToTheAftAreaBeforeCleaning', images)}
                    maxImages={10}
                />

                <MultipleImageUpload
                    id="portVerticalSideNearToTheAftAreaAfterCleaning"
                    label="Port vertical side near to the aft area after cleaning"
                    images={data.portVerticalSideNearToTheAftAreaAfterCleaning || []}
                    onImagesChange={(images) => handleChange('portVerticalSideNearToTheAftAreaAfterCleaning', images)}
                    maxImages={10}
                />
            </div>
...
export default PortVerticalSideForm;